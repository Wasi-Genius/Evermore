...
const Post = ({ post }) => {
	...
	const {
		...
		onError: (commentError) => {
			if (commentError?.message?.includes("2000 characters")) {
				return;
			}
	...
	const { mutate: rePost, isPending: isReposting } = useMutation({
		mutationFn: async (text = "") => apiRequest(`/api/posts/${post._id}/repost`, "POST", { text }),
		onSuccess: () => {
			...
			<div className="flex flex-col flex-1">
				...
				<div className="flex flex-col gap-3 overflow-hidden">
					...
					{post.isRepost && post.repostOf && post.repostOf.user && (
						<RepostContent repostOf={post.repostOf} onImageClick={handleImageClick} />
					)}
				</div>
					...
					<div className="flex gap-4 items-center w-2/3 justify-between">
						...
						<dialog id={`comments_modal${post._id}`} className="modal border-none outline-none">
							<div className="modal-box rounded border border-gray-600 max-w-lg">
								...
								{isError && <p className="text-red-500">{commentError.message}</p>}
							</div>
						...
						{previewOpen && previewImg && (
							<ImageModal img={previewImg} onClose={() => setPreviewOpen(false)} />
						)}
